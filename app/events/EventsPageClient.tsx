...
export default function EventsPageClient({ initialSiteData }: { initialSiteData: ArcSiteData | null }) {
  const siteData = useSiteData(initialSiteData);
  const isLoading = !siteData;
              ...
              <section>
                <div className="mb-5 flex items-end justify-between gap-4">
                  ...
                  <span className="text-sm opacity-55">
                    {isLoading ? "Loading…" : `${upcomingEvents.length} ${upcomingEvents.length === 1 ? "event" : "events"}`}
                  </span>
                </div>
                {isLoading ? (
                  <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3" aria-label="Loading upcoming events">
                    {[0, 1, 2].map((item) => (
                      <div
                        key={item}
                        className="h-64 animate-pulse rounded-2xl border border-black/10 bg-black/[0.04] dark:border-white/10 dark:bg-white/[0.06]"
                      />
                    ))}
                  </div>
